...
export async function createReviewQuery(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, text: string, restaurantName: string, username: string, rating: number) {
    ...
    const randomId = randomNumber.toString() + date_milliSeconds.toString();
    ...
    await g.addE('for').from_(g.addV('review').property('text', text).property('timeStamp', date_milliSeconds).property('rating', rating).property("id", randomId)).to(g.V().hasLabel('restaurant').has('name', restaurantName)).iterate();
...
export async function ReviewRatingQuery(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, username: string, reviewId: string, like_dislike: "like" | "dislike") {
    ...
    return g.addE(like_dislike).from_(g.V().hasLabel('person').has('username', username)).to(g.V().hasLabel('review').has("id", reviewId)).property('timeStamp', date_milliSeconds).iterate()
}
...
export async function findFriendsOfFriendsQuery(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, username: string) {
    ...
    return g.V().hasLabel('person').has('username', username).as('user').out('friendsWith').as('friends').where(gremlin.process.P.neq('user')).out("friendsWith").where(gremlin.process.P.neq('user')).as('friends_friends').select('friends', 'friends_friends').by(gremlin.process.statics.values().fold()).toList();
...
export async function restaurantLatestReviewsQuery(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, name: string) {
    return g.V().hasLabel('restaurant').has("name", name).in_('for').order().by('timeStamp', gremlin.process.order.desc).as('review').in_('writtenBy').as('user').select('review', 'user').by(gremlin.process.statics.valueMap().fold()).by('name').toList();
...
export async function getAllCuisines(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>) {
    return g.V().hasLabel('cuisine').values().toList();

}



export async function getAllRestaurants(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>) {
    return g.V().hasLabel('restaurant').valueMap().toList()

}


export async function getRestaurantInfo(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, name: string) {
    return g.V().hasLabel('restaurant').has('name', name).as('restaurant').out('serves').as('serves').select('restaurant', 'serves').by(gremlin.process.statics.values().fold()).toList()


}


export async function getReviewsLikesDislikes(g: gprocess.GraphTraversalSource<gprocess.GraphTraversal>, restaurantName: string) {
    const likes = await g.V().hasLabel('restaurant').has("name", restaurantName).in_('for').order().by('timeStamp', gremlin.process.order.desc).as('review').inE('like').as('likes').select('likes').by(gremlin.process.statics.values()).group().by(gremlin.process.statics.select('review').by('id')).toList()
    const dislikes = await g.V().hasLabel('restaurant').has("name", restaurantName).in_('for').order().by('timeStamp', gremlin.process.order.desc).as('review').inE('dislike').as('dislikes').select('dislikes').by(gremlin.process.statics.values()).group().by(gremlin.process.statics.select('review').by('id')).toList()

    const data = { likes: likes, dislikes: dislikes }
